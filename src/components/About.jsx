...
const About = () => {
  return (
    <section id="about" className="relative z-10 bg-[#fdf0d5] text-[#0f2c2f] py-20 px-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {skills.map((skill, index) => (
              <div
                key={index}
                className="bg-white flex flex-col items-center justify-center gap-2 py-4 rounded shadow font-medium text-sm"
              >
                {skill.icon}
                {skill.name}
              </div>
